    ...
    <Layout>
      <main className="account-main">
        ...
        {tab === 'companies' && (() => {
          ...
          try { selectedIds = new Set(JSON.parse(localStorage.getItem('ss_companies') || '[]')) } catch {}
          const allCompanies = Object.values(companies).flatMap(cat => cat.list || [])
          const selectedCompanies = allCompanies.filter(c => selectedIds.has(c.id))
          const CATEGORY_ORDER = ['food','cafe','transport','home','clothes','leisure','health','education','travel','other']
          const byCat = CATEGORY_ORDER.map(k => ({
            key: k,
            label: companies[k]?.label,
            list: (companies[k]?.list || []).filter(c => selectedIds.has(c.id)),
          ...
          return (
            <div className="acc-panel">
              ...
              {selectedCompanies.length === 0 ? (
                ...
                <div className="acc-companies-list">
                  {byCat.map(g => (
                    <div key={g.key} className="acc-companies-group">
                      <div className="acc-companies-group-label">{g.label}</div>
                      <div className="acc-companies-group-items">
                        {g.list.map(c => (
                          <div key={c.id} className="acc-company-row">
                            <div className="acc-company-logo" style={{ background: c.color }}>{c.abbr}</div>
                            <span className="acc-company-name">{c.name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
